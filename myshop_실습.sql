...
select oh.order_id , oh.customer_id, oh.order_date, oh.total_due
	from order_header oh right outer join customer c
    on oh.customer_id = c.customer_id
    where c.customer_id in  ('mtkim' , 'odoh' ,'soyoukim','winterkim');
...
select order_id , customer_id, order_date, total_due
	from order_header
    where customer_id in (select customer_id from customer where city = '전주' and (customer_id in  ('mtkim' , 'odoh' ,'soyoukim','winterkim')));
select * from customer
where customer_id = 'odoh';
